...
export const getTeacherTimetable = asyncHandler(async (req, res) => {
  const teacher = await Teacher.findById(req.user._id).populate({
    ...
    // FIX: Include the _id in the selection
    select: '_id subjectName subjectCode semesterNumber'
  });
  ...
  // FIX: Ensure an empty array is returned if no timetable exists
  res.json({
    success: true,
        ...
        if (sem.teacher && sem.teacher.timetable) {
            ...
            if (entry) {
                ...
                todayClasses.push({
                    ...
                    startTime: new Date(`${today.toISOString().split('T')[0]}T${entry.startTime}:00.000Z`).toISOString(),
                    endTime: new Date(`${today.toISOString().split('T')[0]}T${entry.endTime}:00.000Z`).toISOString(),
                });
            }
        }
    });
